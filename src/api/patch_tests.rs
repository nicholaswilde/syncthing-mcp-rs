...
mod tests {
    ...
    #[tokio::test]
    async fn test_patch_folder_config_empty_response() {
        let mock_server = MockServer::start().await;
        let api_key = "test-api-key";
        let folder_id = "test-folder";
        let patch = json!({
        ...
        // 1. PATCH returns 200 OK with empty body
        Mock::given(method("PATCH"))
            .and(path(format!("/rest/config/folders/{}", folder_id)))
        ...
        // 2. Client fallbacks to GET folder
        ...
        assert_eq!(result["label"], "Updated Label");
        assert_eq!(result["type"], "sendonly");
    }

    #[tokio::test]
    async fn test_patch_device_config_empty_response() {
        let mock_server = MockServer::start().await;
        let api_key = "test-api-key";
        let device_id = "TEST-DEVICE-ID";
        let patch = json!({
        ...
        assert_eq!(result["name"], "Updated Device Name");
        assert_eq!(result["addresses"][0], "dynamic");
    }
}
