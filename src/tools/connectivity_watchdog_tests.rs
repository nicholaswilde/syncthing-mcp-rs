...
fn test_monitor_detects_offline_device() {
    ...
    assert!(
        !result.is_offline_too_long,
        "Should not be offline too long immediately after disconnect"
    );
    ...
    assert!(
        result.is_offline_too_long,
        "Monitor should detect offline device after threshold"
    );
    ...
    assert_eq!(
        alerts.len(),
        0,
        "Alert should be cleared after device reconnects"
    );
}
